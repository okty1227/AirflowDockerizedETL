...
def load_json(*args):
    ...
    for job in args:
        ...
        for rw in range(len(raw_df)):
            ...
            postg_hook = PostgresHook(postgres_conn_id='postgres_default')
            
            insert_cmd = f"""
                INSERT INTO postgre_glsd (
...
with DAG(
    ...
    say_start = BashOperator(
        task_id='start_call',
        bash_command="echo start web scraping && pip install selenium"  ## pending: move to dockerfile
    )
    
    job_tasks = []
    for job_name,job_link in read_crawler_conf().items():
        
        exe_start = BashOperator(
            task_id=f'scrape_{job_name}',
            bash_command=f'export job_name="{job_name}" && export job_link="{job_link}"' 
    ...
    say_start >> job_tasks
      ...
      # size mapping
      # salary mapping
      # change skill selector
      
      # load new table
      # ask js to load new table and render to js
     
    # say_start >> init_postgre >> load_data
    # say_start >> job_tasks
    # init_postgre >> job_tasks
    # job_tasks >> load_data